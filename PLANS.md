...
- **OpenWork-style UX simplification (2026-04-09):** Settings **primary** = workspace + Save; **Advanced** = limits, browser CDP, inline tool visibility, clear saved workspace permissions, diagnostics. Header tool-view toggle removed (default show tool steps in chat). Composer Mode dropdown + Templates in a disclosure; lighter context-panel copy; footer no longer repeats full workspace path. Logged: `docs/IMPLEMENTATION.md` **2026-04-09 Desktop UI: OpenWork-style simplification**.
