...
- **OpenWork-style UX (current):** Three-pane shell after first use; **zero-state** is a dedicated onboarding shell with a **Preflight** block (workspace, Copilot sign-in, CDP reachability, default work mode) plus the first request composer. Sidebar / context panel / footer stay hidden until the first conversation starts. The app is now **conversation-based**: `start_agent` creates a new conversation, `continue_agent_session` appends to the active idle conversation, and mode changes require **New conversation**. Context tabs are **Plan | Integrations**; tool **policy summary** folds under Plan. Tool steps **always** inline in chat with humanized summaries. Settings exposes workspace, default work mode, browser/CDP hints, `maxTurns`, `always on top`, reconnect, and diagnostics export. Approvals: **Allow once** / **Always allow in this conversation** / optional **Always allow in this folder**. **Ctrl+Enter** (**⌘+Enter**) sends; **Enter** newline. Milestone log: `docs/IMPLEMENTATION.md` **2026-04-12 Desktop UI: conversation model + preflight settings refresh**.
