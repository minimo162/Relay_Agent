...
- Node CDP bridge: `apps/desktop/src-tauri/binaries/copilot_server.js` — requires `relay_session_id` / `relay_request_id`, keeps one Copilot tab/thread per Relay session, accepts optional `relay_new_chat`, and scopes `POST /v1/chat/abort` to the matching request id. Mutable endpoints require `X-Relay-Boot-Token`.
