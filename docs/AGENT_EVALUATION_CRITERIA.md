...
**Implementation note:** The Node `copilot_server.js` path (HTTP `/v1/chat/completions`) now isolates **one Copilot tab per Relay session**. Requests must carry `relay_session_id` and `relay_request_id`; the bridge opens a fresh Copilot chat on first use / recovery, and the Rust CDP agent also requests a fresh Copilot chat for normal follow-ups that include successful Relay Tool Result evidence. `relay_new_chat: true` and `RELAY_COPILOT_NEW_CHAT_EACH_TURN=1` still exist for explicit resets, but the default is no longer “one global current thread for every session”.
