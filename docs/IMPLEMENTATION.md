...
**Verification:** `pnpm exec tsc -p tsconfig.json --noEmit` (apps/desktop) — pass. `CI=1 E2E_SKIP_AUTH_SETUP=1 pnpm exec playwright test tests/app.e2e.spec.ts tests/e2e-comprehensive.spec.ts` (apps/desktop) — **54 passed** (2026-04-09). *Note:* with `reuseExistingServer`, stale `vite preview` can serve an old bundle; use `CI=1` or restart preview after changing `tests/tauri-mock-core.ts`.
