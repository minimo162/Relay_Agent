...
public sealed class RelayCopilotChatClient(ICopilotTransport transport) : IChatClient
{
    ...
    private static string BuildToolProjectionPrompt(ChatOptions? options, RelayAdmissibleActionEnvelope? envelope)
    {
        var parts = new List<string>
        {
            ...
            "For PDF proofreading, typo checks, notation checks, or two-PDF comparison, call read on every exact PDF before final. Ground findings only in extracted text snippets and mention text-layer/OCR limits when extraction is incomplete.",
