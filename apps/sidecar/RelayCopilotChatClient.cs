...
public sealed class RelayCopilotChatClient(ICopilotTransport transport) : IChatClient
{
    ...
    private static string BuildToolProjectionPrompt(ChatOptions? options, RelayAdmissibleActionEnvelope? envelope)
    {
        var parts = new List<string>
        {
            ...
            "For file search, use only visible generic tools: glob for candidate filenames, read for exact Office/PDF/document candidates, and grep for plaintext/code content. Do not suggest external retrievers such as biling_retriever/vector search unless they appear in Tools.",
            ...
            "If grep returns zero matches, broaden or change the terms and grep again before read/final. If glob finds Office/PDF/document candidates, read exact candidates before saying no relevant local file was found. Do not read README.md as a generic fallback unless the user asked for project instructions or README content.",
