...
> **Relay Agent implementation:** CSS variables and component classes live in [`src/index.css`](src/index.css). **Typography:** `.ra-type-*` map §3 — display (with §8 breakpoint letter-spacing), body sans/serif/mono, **button label** / **button caption** (`ss09`), **caption**, **system** heading/caption/micro (`--ra-font-ui`), optional **Lato** stack (`--ra-font-lato`, font not bundled). **Radii:** `--ra-radius-micro` through `--ra-radius-featured` (plus `--ra-radius-pill`). **Links:** `--ra-link-*` on `body a`; assistant markdown links stay under [`.ra-md-assistant`](src/index.css). **Buttons:** `tertiary` pill (Surface 500), ghost padding per §4, `.ra-button` + type utility combos in CSS. **Shell:** sticky, blurred `.ra-shell-header`. **Light** theme uses Surface / border / interaction values here; **dark** uses the same token names with a paired warm-charcoal scale. The app **defaults to light** (`<html data-theme="light">`, `localStorage` `relay-agent/theme`). Brand font **names** are first in stacks; files are **not** shipped — fallbacks match §3. Tailwind fragments: [`src/lib/ui-tokens.ts`](src/lib/ui-tokens.ts). Verification: [`../../docs/IMPLEMENTATION.md`](../../docs/IMPLEMENTATION.md).
