...
test("AionUi Relay manifest reuses AionUi core conversation UX", () => {
  ...
  assert.deepEqual(manifest.aionUiCoreUx, {
    ...
    rules: [
      ...
      "Expose search as curated assistant/skill entries, GuidPage input/workspace controls, workspace quick filter/status, and preview-linked results.",
      "Keep AionUi GuidPage, conversation tabs, workspace panel, file mentions, and preview panel as the primary user-facing controls.",
...
test("AionUi Relay manifest defines the GuidPage beginner search flow", () => {
  ...
  assert.deepEqual(manifest.guidBeginnerUx, {
    ...
    rules: [
      ...
      "Hide provider/model setup, tool settings, WebUI/channel setup, extension settings, Skills Market, model switchers, permission-mode controls, detected-agent selectors, preset edit controls, backend switchers, speech input, slash commands, skill indicators, token usage meters, and assistant-management entrypoints from beginner views.",
