...
fn run_relay_document_search_blocking(
    ...
    let input = json!({
        ...
        "options": {
            ...
            "indexDbPath": cache_root.join("search.sqlite").display().to_string(),
            "parsedDocumentCacheDir": cache_root.join("parsed-documents").display().to_string(),
            "derivedContentIndexDir": cache_root.join("derived-content").display().to_string(),
            "indexCoordinatorDir": cache_root.join("index-coordinator").display().to_string(),
            "failureRegistryDir": cache_root.join("failures").display().to_string(),
