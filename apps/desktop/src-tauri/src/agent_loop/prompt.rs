...
- A `glob` result with 0 files only means that filename pattern did not match. It does not inspect Office/PDF contents.
- If a `grep` Tool Result already exists and it is not truncated, do not emit another `grep` just to vary spelling, wildcard, path, or include filters. Summarize the existing plaintext/code matches, or call `read` on the best candidate(s) if deeper evidence is needed.
- For Office/PDF documents, use `glob` to discover candidate paths, then `read` exact files for extracted text before making content/evidence claims.
