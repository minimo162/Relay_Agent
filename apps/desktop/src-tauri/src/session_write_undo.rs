//! Per-session stacks for undo/redo of successful workspace file mutations (OpenCode-style `/undo`).
//! One stack entry per mutating tool invocation (`write_file`, `edit_file`, `NotebookEdit`, PDF tools).
...
/// Restore `path` to `previous` contents; `None` means the path did not exist before the mutation.
#[derive(Debug, Clone)]
pub(crate) enum PathUndoOp {
    Restore {
        path: PathBuf,
...
pub(crate) struct WriteUndoStacks {
    undo: Vec<Vec<PathUndoOp>>,
    redo: Vec<Vec<PathUndoOp>>,
}

impl WriteUndoStacks {
    pub(crate) fn push_mutation(&mut self, ops: Vec<PathUndoOp>) {
        if ops.is_empty() {
            return;
        }
        self.redo.clear();
...
/// Capture pre-mutation snapshots for tools we support. Returns `None` if nothing to record.
pub(crate) fn snapshots_before_mutation(tool_name: &str, input: &Value) -> Option<Vec<PathUndoOp>> {
    let mut out = Vec::new();
    match tool_name {
