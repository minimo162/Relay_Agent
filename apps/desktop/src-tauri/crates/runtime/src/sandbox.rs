...
/// Check whether `unshare --user` works on this system (claw-code sandbox lane: capability probe).
/// The binary may exist while user namespaces are disabled (e.g. some CI), which would cause silent failures.
