...
pub async fn pull_rust_diagnostics(
    ...
    timeout(IO_TIMEOUT, write_msg(&mut stdin, &init))
        .await
        .map_err(|_| "LSP initialize timed out".to_string())?
        .map_err(|e| e.to_string())?;

    let init_resp = timeout(IO_TIMEOUT, next_json_response(&mut reader))
        .await
    ...
    timeout(IO_TIMEOUT, write_msg(&mut stdin, &notif))
        .await
    ...
    timeout(IO_TIMEOUT, write_msg(&mut stdin, &did_open))
        .await
    ...
    timeout(IO_TIMEOUT, write_msg(&mut stdin, &diag_req))
        .await
        .map_err(|_| "LSP diagnostic request timed out".to_string())?
        .map_err(|e| e.to_string())?;

    let diag_resp = timeout(IO_TIMEOUT, next_json_response(&mut reader))
        .await
