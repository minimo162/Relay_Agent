...
impl SystemPromptBuilder {
    ...
    pub fn build(&self) -> Vec<String> {
        ...
        sections.extend(claw_style_discipline_sections());
...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "When read output, Tool Result content, or file text in the session bundle is present, treat it as the source of truth for that file: claims about syntax, structure, bugs, or symbols must be traceable to those strings (quote a short substring, cite line numbers from the tool output, or paraphrase only what appears there).".to_string(),
        "If a file is large or you only saw a slice, say so; use read with offset/limit (line-based) or a follow-up read before claiming behavior in parts you have not read.".to_string(),
    ]);
...
/// Claw-style harness sections (`# System`, `# Doing tasks`, `# Executing actions with care`) for reuse outside `SystemPromptBuilder`.
#[must_use]
pub fn claw_style_discipline_sections() -> Vec<String> {
    vec![
...
",
    ...
    #[test]
    fn renders_claw_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".claw")).expect("claw dir");
    ...
    fn discovers_dot_claw_instructions_markdown() {
        ...
        fs::write(
            nested.join(".claw").join("instructions.md"),
            "claw instruction markdown",
        )
        .expect("write .claw instructions.md");
