...
use crate::opencode_tools::catalog::{
    self as opencode_tool_catalog, CdpPromptToolSpec, OpencodeToolPermissionMode as PermissionMode,
};
...
type ParsedToolCall = (String, String, String);
type ParsedToolCalls = Vec<ParsedToolCall>;
type TopLevelToolPayload = (String, ParsedToolCalls);
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DesktopPermissionPolicy {
    active_mode: PermissionMode,
    tool_requirements: BTreeMap<String, PermissionMode>,
}

impl DesktopPermissionPolicy {
    #[must_use]
...
#[must_use]
pub fn desktop_permission_policy() -> DesktopPermissionPolicy {
    let mut policy = DesktopPermissionPolicy::new(PermissionMode::WorkspaceWrite);
    for spec in opencode_tool_catalog::desktop_tool_permission_requirements() {
        policy = policy.with_tool_requirement(spec.name, spec.required_permission);
    }
    policy
}
...
fn tool_permissions() -> Vec<SessionToolPermissionRow> {
    ...
    opencode_tool_catalog::mvp_tool_names()
        .into_iter()
        .map(|name| {
            let required = policy.required_mode_for(name);
            let requirement = classify_permission_ui_requirement(host, required);
            let reason = describe_permission_reason(name, host, required, requirement);
...
fn cdp_catalog_specs_for_flavor(catalog_flavor: CdpCatalogFlavor) -> Vec<CdpPromptToolSpec> {
    #[cfg(test)]
    ensure_test_cdp_catalog_runtime();

    let specs = opencode_tool_catalog::cdp_prompt_tool_specs();
...
#[cfg(test)]
fn ensure_test_cdp_catalog_runtime() {
    static INIT: std::sync::Once = std::sync::Once::new();
    INIT.call_once(|| {
        std::env::set_var("RELAY_OPENCODE_TOOL_RUNTIME_URL", "http://127.0.0.1:1");
    });
}
...
fn render_cdp_tool_entry(spec: &CdpPromptToolSpec) -> String {
...
#[must_use]
pub fn parse_copilot_tool_response(raw: &str, parse_mode: CdpToolParseMode) -> TopLevelToolPayload {
        ...
        let text: Vec<String> = msg
            ...
            .map(|b| match b {
                ContentBlock::Text { text } => {
                    match msg.role {
                        MessageRole::User => user_text_chars += text.len(),
                        MessageRole::Assistant => assistant_text_chars += text.len(),
                        _ => {}
                    }
                    text.clone()
                }
...
#[must_use]
pub fn build_cdp_prompt(request: &CdpPromptRequest<'_>) -> String {
...
fn cdp_json_fence_whitelist() -> HashSet<String> {
    opencode_tool_catalog::cdp_json_fence_tool_names()
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
fn dedupe_relay_tool_calls(calls: ParsedToolCalls) -> ParsedToolCalls {
    let mut seen = HashSet::new();
...
fn parse_tool_payloads(payloads: &[String], whitelist: &HashSet<String>) -> ParsedToolCalls {
    let mut out = Vec::new();
    for p in payloads {
...
fn parse_one_tool_call(v: &Value) -> Option<ParsedToolCall> {
    let obj = v.as_object()?;
...
fn collect_summary_assistant_text(summary: &CdpTurnSummary) -> String {
...
#[must_use]
pub fn core_error_needs_forced_compaction(error: &DesktopCoreError) -> bool {
...
```"#;
    ...
    fn later_turn_meta_stall_gets_continue_nudge_with_budget_remaining() {
        let s = summary("Lining things up...", Vec::new(), TurnOutcome::Completed);
        assert_eq!(
    ...
    fn pathless_html_tetris_request_with_full_document_body_escalates_to_write_repair() {
        ...
        .join("\n");
        let s = summary(&body, Vec::new(), TurnOutcome::Completed);
        let decision = decide_loop_after_success(
            "htmlでテトリスを作成して",
            "htmlでテトリスを作成して",
            0,
